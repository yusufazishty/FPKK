...
if os.stat("weight_log.txt").st_size != 0:
...
for x in range(len(content)):
	...
	if winner==dist_a:
		...
		for b in range(7):
			weight[0+1][b] = weight[0+1][b] + lrate * (input_lyr[b]-weight[0+1][b])
	else:
		for b in range(7):
			weight[0+2][b] = weight[0+2][b] + lrate * (input_lyr[b]-weight[0+2][b])
...
file.write('\r\n')
file.write(str(v[0][0])+'\t'+str(v[1][0])+'\t'+str(v[2][0])+'\t'+str(v[0][1])+'\n'+str(v[1][1])+'\t'+str(v[2][1]))
file.close()
